...
function efectoHabilidades(){
    ...
    if(distancia_skills>=300 && entro==false){
        ...
        const intervalJava = setInterval(function(){
            pintarBarra(java, 15, 1, intervalJava);
        },100);
        const intervalJquery = setInterval(function(){
            pintarBarra(jquery, 14, 3, intervalJquery);
        },100);
        const intervalPhyton = setInterval(function(){
            pintarBarra(phyton, 16, 0, intervalPhyton);
        },100);
        const intervalPhp = setInterval(function(){
            pintarBarra(php, 14, 2, intervalPhp);
        },100);
    }
}
...
function pintarBarra(id_barra, cantidad, indice, interval){
    ...
    if(x < cantidad){
        ...
        elementos[x].style.backgroundColor = "#022994";
    }else{
